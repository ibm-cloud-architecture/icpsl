...
function configure_masters {
  ...
  ansible-playbook -v -i $HOSTS ansible/icp-master.yaml -e "master_ip=$MASTER1_IP"
}
...
function configure_nodes {
  ...
  # Execute kube-master playbook
  ansible-playbook -v -i $HOSTS ansible/icp-node.yaml --extra-vars "master_ip=$MASTER1_IP"
}
